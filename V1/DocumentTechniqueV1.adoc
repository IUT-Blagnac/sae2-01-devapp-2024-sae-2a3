...
**__PrelevementAutomatique__**

Représente les prélèvements automatiques effectués sur les comptes courants.

*** Attributs : 

**** idPrelevement : identifiant unique pour chaque prélèvelement automatique 
...
** ChefAgence (0..1) -- 1 AgenreceBancaire : une agence est gérée par un chef d'agence. 
...
CompteCourant () -- () PrevelementAutomatique : les prélèvements automatiques sont effectués sur les comptes des clients. Cependant, cette liaison (par composition) montre que la suppression du compte courant entraîne la suppression de tous les prélèvements automatiques associés.
